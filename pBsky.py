...
def main():
    ...
    parser.add_argument(
        "-d", "--delete", nargs=2, help="delete a post with given did and rkey."
    )
    parser.add_argument(
        "-g",
        "--get",
    ...
    parser.add_argument(
        "-f", "--follow", type=str, help="follow a user with a given username"
    )
    parser.add_argument(
        "-gp", "--getprofile", type=str, help="get a profile for a user"
    )
    ...
    if args.getprofile != None:
        get_profile(session, args.getprofile)  # handle

    # Post a Skeet
    if args.post != None:
        post_skeet(session, args.post)  # text to post

    # Reply to Skeet
    if args.reply != None:
        reply_to_skeet(
            session, args.reply[0], args.reply[1], args.reply[2]  # text to post  # did
        )  # rkey

    # Delete Skeet
    if args.delete != None:
        delete_skeet(session, args.delete[0], args.delete[1])  # did  # rkey

    # Follow Handle
    if args.follow != None:
        follow_handle(session, args.follow)  # handle

    # Get Skeets
    if args.get != None:
        if args.get[0].isnumeric():
            get_skeets(session, args.get[0], "")  # count  # placeholder
        else:
            get_skeets(session, args.get[0], args.get[1])  # handle  # count
...
def get_profile(my_session, user_name):
    ...
    print_follows_followers_posts_bar(
        profile_followsCount, profile_followersCount, profile_postsCount
    )
    print_fat_divider()
...
def reply_to_skeet(my_session, skeet_text, did, rkey):
    ...
    root = {"cid": original_post[0].get("cid"), "uri": original_post[0].get("uri")}
    parent = {"cid": original_post[0].get("cid"), "uri": original_post[0].get("uri")}
...
def get_skeets(my_session, arg0, arg1):
    ...
    for i in feed:
        ...
        if str(i.get("post").get("record").get("reply")) != "None":
            bloot_reply_uri = str(
                i.get("post").get("record").get("reply").get("parent").get("uri")
            )
            ret_json = my_session.getBlootByUrl(bloot_reply_uri).json().get("posts")
            bloot_response_author_handle = str(
                ret_json[0].get("author").get("displayName")
            )
            print_reply_to(bloot_response_author_handle)

            orig_bloot_text = clean(
                str(i.get("reply").get("parent").get("record").get("text"))
            )
            orig_bloot_displayName = str(
                i.get("reply").get("parent").get("author").get("displayName")
            )
            orig_bloot_handle = str(
                i.get("reply").get("parent").get("author").get("handle")
            )
            orig_bloot_did = str(
                i.get("reply").get("parent").get("author").get("did")[8:]
            )
            orig_bloot_uri = str(i.get("reply").get("parent").get("uri")).split("/")[-1]
            orig_bloot_replyCount = str(i.get("reply").get("parent").get("replyCount"))
            orig_bloot_repostCount = str(
                i.get("reply").get("parent").get("repostCount")
            )
            ...
            if str(i.get("reply").get("parent").get("embed")) != "None":
                ...
                if (
                    i.get("reply").get("parent").get("embed").get("$type")
                    == "app.bsky.embed.images#view"
                ):
                    orig_bloot_images = (
                        i.get("reply").get("parent").get("embed").get("images")
                    )
                ...
                if (
                    ...
                    orig_embedded_description = clean(
                        str(
                            i.get("reply")
                            .get("parent")
                            .get("embed")
                    ...
                    print_embedded_website(
                        orig_embedded_title,
                        orig_embedded_description,
                        orig_embedded_uri,
                    )
            ...
            print_reply_repost_like_bar(
                orig_bloot_replyCount, orig_bloot_repostCount, orig_bloot_likeCount
            )
        ...
        if str(i.get("reason")) != "None":
            bloot_repost_author_displayName = str(
                i.get("reason").get("by").get("displayName")
            )
        ...
        if str(i.get("post").get("embed")) != "None":
            ...
            if i.get("post").get("embed").get("$type") == "app.bsky.embed.record#view":
                ...
                embedded_handle = (
                    i.get("post").get("embed").get("record").get("author").get("handle")
                )
                print_embedded_post(
                    embedded_handle, embedded_displayName, embedded_text
                )

            # Embedded website found
            if (
                i.get("post").get("embed").get("$type")
                == "app.bsky.embed.external#view"
            ):
                embedded_description = clean(
                    str(i.get("post").get("embed").get("external").get("description"))
                )
                embedded_title = clean(
                    str(i.get("post").get("embed").get("external").get("title"))
                )
                embedded_uri = i.get("post").get("embed").get("external").get("uri")
                print_embedded_website(
                    embedded_title, embedded_description, embedded_uri
                )
        ...
        # Reply, Repost, and Like bar
        print_reply_repost_like_bar(
            bloot_replyCount, bloot_repostCount, bloot_likeCount
        )
